...
class _AdminPartnersScreenState extends State<AdminPartnersScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      setState(() {
        _partners =
            ((res.data as Map)['partnerships'] as List?)?.cast<dynamic>() ?? [];
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Future<void> _invite() async {
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(
                labelText: 'Email pengepul',
                hintText: 'pengepul@reloop.id',
              ),
    ...
    if (result == null || (result['email']?.isEmpty ?? true) || !mounted) {
      return;
    }
    try {
      ...
      children: [
        ...
        if (filtered.isEmpty)
          ...
          ...filtered.map((p) {
            ...
            final serviceArea =
                partner['serviceAreaJson'] as Map<String, dynamic>?;
            final regions =
                (serviceArea?['regions'] as List?)?.join(', ') ?? '';
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                (collector?['name'] as String?) ?? 'Pengepul',
                                style: TextStyle(
