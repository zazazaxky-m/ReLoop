    ...
    def sync_media(self) -> bool:
        ...
        try:
            ...
            if not manifest.get("enabled"):
                self.db.set_json("media_playlist", {"enabled": False, "version": manifest.get("version", ""), "items": []})
            ...
            for item in items:
                ...
                if not extension:
                    raise RuntimeError("Format media pada manifest tidak didukung")
                filename = f"{item['id']}{extension}"
                expected_names.add(filename)
                target = cache_dir / filename
                valid_existing = False
                if target.is_file() and target.stat().st_size == int(item.get("fileSize") or 0):
                    valid_existing = hashlib.sha256(target.read_bytes()).hexdigest() == item.get("sha256")
                if not valid_existing:
                    data = self._signed_get(str(item["downloadPath"]), timeout=60)
                    if len(data) != int(item.get("fileSize") or 0):
            ...
            for stale in cache_dir.iterdir():
                if stale.is_file() and stale.name not in expected_names and stale.name != ".gitkeep":
                    stale.unlink(missing_ok=True)
            self.db.set_json("media_playlist", {"enabled": True, "version": manifest.get("version", ""), "items": local_items})
            self.online = True
            self.last_sync_at = time.time()
            self.last_error = None
            return True
